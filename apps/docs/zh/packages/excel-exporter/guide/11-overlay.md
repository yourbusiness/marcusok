...
遮罩共用一个 DOM 节点并做引用计数：并发导出渲染进同一个遮罩（最后更新者为准），最后一个关闭时才移除。并发调用 `exportTable` / `exportExcel` 是安全的，多次运行之间不会互相残留。渲染内容（标题、提示、文案、进度）始终属于最近一次 `show` / progress / phase 事件的调用方——包括延迟揭开定时器到点时其发起导出已经 close 的情况。共享节点带来两个结果：遮罩尚未显示时第二个导出介入，它不会获得独立的 `delayMs` 门控（揭开时机跟随首个调用方的定时器）；遮罩已显示时第二个导出介入，它在自己的第一个 progress/phase 事件起接管显示。
