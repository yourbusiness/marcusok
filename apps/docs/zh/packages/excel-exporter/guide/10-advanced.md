...
工作表名需满足 ECMA-376 约束：非空、不超过 31 字符、不含 `: \ / ? * [ ]`、不得以单引号开头或结尾，且在 `sheets` 内不得重名——重名与违规一样走统一报错，而不会生成损坏文件或被静默改名。违反时不会生成损坏文件，也不会以异常形式抛出——校验错误会被捕获并走兜底流程，兜底路径会再次校验同一表名，最终以 `{ success: false, error }` 返回（错误信息明确）。
