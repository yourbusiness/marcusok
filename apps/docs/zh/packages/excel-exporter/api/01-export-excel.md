...
- `exportTable(options)`：常见表格数据便捷导出，支持 AntD `title`/`dataIndex` 与 Element Plus `label`/`prop`；sheet 名默认 `"Sheet1"`（可用 `sheetName` 覆盖）；`freezeRows` / `autoFilter` / `merges` 会透传给 sheet；
